...
CREATE TABLE `data_game_start` (
  ...
  `user_id` varchar(50) DEFAULT '' COMMENT '账户（userId）',
  `platform_terminal` int(11) NOT NULL DEFAULT '1' COMMENT '终端:1.pc 2.安卓 3ios 4.h5 5.其他',
  `platform_id` int(11) NOT NULL DEFAULT '1' COMMENT '平台Id:默认为玩一下平台;PC=1,移动端为31111',
...
CREATE TABLE `data_installs` (
  ...
  `platform_terminal` int(11) NOT NULL DEFAULT '2' COMMENT '终端:2.安卓 3.ios',
  `country` int(11) NOT NULL DEFAULT '0' COMMENT '国家:0中国 1美国 2泰国 3越南 4韩国 5印尼6繁体7新加坡8马来西亚',
  `longitude` decimal(14,5) NOT NULL DEFAULT '0.00000' COMMENT '经度',
  `latitude` decimal(14,5) NOT NULL DEFAULT '0.00000' COMMENT '纬度',
...
CREATE TABLE `data_interface_invoke` (
  ...
  `platform_id` int(11) NOT NULL DEFAULT '1' COMMENT '平台Id:默认为玩一下平台;PC=1,移动端为31111',
  `platform_terminal` int(11) NOT NULL DEFAULT '1' COMMENT '终端:1.pc 2.安卓 3ios 4.h5 5.其他',
  `interface_type` int(11) NOT NULL DEFAULT '1' COMMENT '接口类型:1.注册接口 2.充值接口',
  `website_domain` varchar(50) NOT NULL DEFAULT '' COMMENT '域名',
  `url_path` varchar(300) NOT NULL DEFAULT '' COMMENT '网站路径，不带参数,不计域名',
  `create_time` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建日期',
  PRIMARY KEY (`id`)
...
CREATE TABLE `data_view` (
  ...
  `user_id` varchar(50) DEFAULT NULL COMMENT '账户（userId）',
  `platform_terminal` int(11) NOT NULL DEFAULT '1' COMMENT '终端:1.pc 4.h5 5.其他',
  `website_domain` varchar(50) NOT NULL DEFAULT '' COMMENT '域名',
  `page_view` int(11) NOT NULL DEFAULT '1' COMMENT '浏览页面: 1.首页 2.注册 3.充值',
  `url_path` varchar(300) NOT NULL DEFAULT '' COMMENT '网站路径，不带参数,不计域名',
  `create_time` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建日期',
  PRIMARY KEY (`id`)
