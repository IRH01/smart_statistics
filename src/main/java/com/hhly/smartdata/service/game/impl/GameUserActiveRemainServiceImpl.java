...
import com.hhly.smartdata.mapper.game.operative.GameUserActiveRemainReposity;
...
@Service
public class GameUserActiveRemainServiceImpl implements GameUserActiveRemainService{

    @Autowired
    GameUserActiveRemainReposity gameUserActiveRemainReposity;

    @Override
    public JSONObject find(Map<String, Object> conditionMap, int pageNumber,
                           int pageSize){
        PageHelper.startPage(pageNumber, pageSize);
        List<GameUserActiveRemain> values = gameUserActiveRemainReposity.find(conditionMap);
        PageInfo<GameUserActiveRemain> pageInfo = new PageInfo<GameUserActiveRemain>(values);
    ...
    public boolean canExport(Map<String, Object> conditionMap){
        int count = gameUserActiveRemainReposity.findCount(conditionMap);
    ...
    private String exportZip(String fileName, int pageCount, Map<String, Object> conditionMap, int type){
        try{
            ...
            for(int pageNumber = 1; pageNumber <= pageCount; pageNumber++){
                List<GameUserActiveRemain> valueList = this.findByPage(conditionMap, 1, FileConstants.EXCEL_SINGLE_SHEET_ROW_NUMBER, type);
    ...
    private List<Object> getQValueList(GameUserActiveRemain value){
...
//		valueList.add(value.getName());
    ...
    private List<GameUserActiveRemain> findByPage(Map<String, Object> conditionMap, int pageNumber, int pageSize, int type){
        PageHelper.startPage(pageNumber, pageSize);
        List<GameUserActiveRemain> values = gameUserActiveRemainReposity.find(conditionMap);
        return values;
    }
    ...
    public List<GameUserActiveRemain> getProduces(Map<String, Object> condition){
        ...
        return gameUserActiveRemainReposity.getProduces(condition);
    }
