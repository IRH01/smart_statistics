...
<script type="text/javascript">
     ...
     $(function() {
		...
		$("#companyName").autocomplete(
				{
					source : "<c:url value="/admin/cp/cpCompanyNameAutocomplete.do"/>",//数据来源
		...
		/* //添加渠道名称自动完成
 		$("#channel").autocomplete(
 				{
 					//source : "/sys/channel/autocomplete.do"+"?"+Math.random(),//数据来源
 					source : "<c:url value="/sys/channel/autocomplete.do"/>",//数据来源
 					minLength : 1, //最小输入多少个字符触发查询
 					select : function(event, ui) {//选择该记录时执行
	    ...
	    $( "#channel" ).autocomplete({
	    	source: function( request, response ) {
	    		...
	    		var cpId = $("#cpId").val();
	    		$.getJSON( "<c:url value="/sys/channel/autocomplete.do"/>?cpId="+cpId, request, function( data, status, xhr ) {
	    			response( data );
	    		});
	    		noclean=false;
	    	},//数据来源
					...
					<form name="form" class="form-inline"
						...
						<fieldset>
							<div class="form-group form-group-sm right20">
								<label class="control-label" for="name">CP名称：</label> <input
									class="form-control" type="text" id="companyName" name="cpName"
									value="${channel.cpName}"> <input type="hidden"
									id="cpId" />
							</div>
							<div class="form-group form-group-sm right20">
								<label class="control-label" for="name">渠道名称：</label> <input
