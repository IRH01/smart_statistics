									...
									<div class="titleDiv" style="height:25px;">
									    ...
									    <div class="select-fr" style="padding-right: 30px;float:left;">
									    	<table style="margin-left:5px;">
									    		<tr>
									    			<td class="tb1Td"><span class="span">账号：</span></td>
									    			<td><input id="accountNo" class="input"></td>
													<td class="tb1Td"><span class="span">推广编号：</span></td>
									    			<td><input id="promotNo" class="input"></td>
...
<script type="text/javascript">
			...
			var bindEnterEventOfAllEle = function(){
				bindEnterEvent("accountNo",search);
				bindEnterEvent("promotNo",search);
			}
			
	   		var pageSize = 10;
			var addTbRow = function(data,isTotalRow){
				if(null != data && undefined != data && "" != data){
					var ele="<tr><td>" + data.accountNo + "</td><td>" + data.promotNo + "</td><td><a href='${path}/agent/channel/showChannelDetails.do?accountNo=" + data.accountNo + "&promotNo=" + data.promotNo + "'> " + data.channelNo + "</a></td><td>" + data.registAmount + "</td><td>" + data.incomeSum + "</td><td>" + data.startTimes + "</td><td>" + data.avguseTime + "</td>";
					$("#data").append(ele);
				}
			}
			...
			var showData = function(pageNumber,pageSize){
				$("#data").empty();
				$.post("${path}/agent/channel/list.do",{
					accountNo:$("#accountNo").val(),
					promotNo:$("#promotNo").val(),
						...
						if(null == infoData || undefined == infoData || 0 >= infoData.length){
							...
							for(var i=0;i < infoData.length;i++){
								var ele = {
										...
										channelNo:infoData[i].channelNo,
										registAmount:infoData[i].registAmount,
										incomeSum:infoData[i].incomeSum,
										startTimes:infoData[i].startTimes,
										avguseTime:infoData[i].avguseTime
								}
								addTbRow(ele);
							}
						}
