									    ...
									    <div class="select-fr" style="padding-right: 30px;float:left;">
									    	<table style="margin-left:5px;">
									    		<tr>
													...
													<td class="tb1Td"><span class="span">注册量：</span></td>
									    			<td><input id="regCountStart" class="input">~<input id="regCountEnd" class="input"></td>
									    			<td class="tb1Td"><span class="span">充值金额：</span></td>
									    			<td><input id="rechargeAmountStart" class="input">~<input id="rechargeAmountEnd" class="input"></td>
									    			
												</td>
									    		</tr>
									    		
									    		<tr>
									    			<td class="tb1Td"><span class="span">启动次数：</span></td>
									    			<td><input id="startTimeStart" class="input">~<input id="startTimeEnd" class="input"></td>
									    			<td class="tb1Td"><span class="span">平均使用时长：</span></td>
									    			<td><input id="avgTimeStart" class="input">~<input id="avgTimeEnd" class="input"></td>
...
<script type="text/javascript">
			...
			var bindEnterEventOfAllEle = function(){
				...
				bindEnterEvent("regCountStart",search);
				bindEnterEvent("regCountEnd",search);
				bindEnterEvent("rechargeAmountStart",search);
				bindEnterEvent("rechargeAmountEnd",search);
				bindEnterEvent("startTimeStart",search);
				bindEnterEvent("startTimeEnd",search);
				bindEnterEvent("avgTimeStart",search);
				bindEnterEvent("avgTimeEnd",search);
			}
			
	   		var pageSize = 10;
			var addTbRow = function(data,isTotalRow){
				if(null != data && undefined != data && "" != data){
					var ele="<tr><td>" + data.channelName + "</td><td><a href='${path}/agent/channel/showChannelCostDetails.do?accountNo=" + data.accountNo + "&promotNo=" + data.promotNo + "&channelId=" + data.channelId + "'> " + data.regCount + "</a></td><td><a href='${path}/agent/channel/showChannelRegistDetails.do?accountNo=" + data.accountNo + "&promotNo=" + data.promotNo + "&channelId=" + data.channelId + "'> " + data.rechargeAmount + "</a></td><td>" + data.startTime + "</td><td>" + data.avgTime + "</td>";
					$("#data").append(ele);
				}
			}
			...
			var showData = function(pageNumber,pageSize){
				$("#data").empty();
				$.post("${path}/agent/channel/listChannelDetails.do",{
					promotNo:${param.promotNo},
					accountNo:${param.accountNo},
					channelName:$("#channelName").val(),
					regCountStart:$("#regCountStart").val(),
					regCountEnd:$("#regCountEnd").val(),
					rechargeAmountStart:$("#rechargeAmountStart").val(),
					rechargeAmountEnd:$("#rechargeAmountEnd").val(),
					startTimeStart:$("#startTimeStart").val(),
					startTimeEnd:$("#startTimeEnd").val(),
					avgTimeStart:$("#avgTimeStart").val(),
					avgTimeEnd:$("#avgTimeEnd").val(),
						...
						if(null == infoData || undefined == infoData || 0 >= infoData.length){
							...
							for(var i=0;i < infoData.length;i++){
								var ele = {
										...
										accountNo:infoData[i].accountNo,
										promotNo:infoData[i].promotNo,
										...
										startTime:infoData[i].startTime,
										avgTime:infoData[i].avgTime
								}
								addTbRow(ele);
							}
						}
			...
			var exportToExcel = function(){
				...
				$.ajax({
				    url: "${path}/agent/channel/export.do",
				    type: "POST",
				    data: {
				    	promotNo:${param.promotNo},
						accountNo:${param.accountNo},
				    ...
				    success:function(response, status, request){
				        ...
				        window.location = "${path}/agent/channel/download.do?uuid=" + response.uuid;
			    }});
			}
			...
			var reset = function(){
				...
				$("#regCountStart").val("");
				$("#regCountEnd").val("");
				$("#rechargeAmountStart").val("");
				$("#rechargeAmountEnd").val("");
				$("#startTimeStart").val("");
				$("#startTimeEnd").val("");
				$("#avgTimeStart").val("");
				$("#avgTimeEnd").val("");
