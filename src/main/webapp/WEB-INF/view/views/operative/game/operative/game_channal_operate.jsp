...
<script type="text/javascript">
			...
			var addTbRow = function(data,isTotalRow){
				if(null != data && undefined != data && "" != data){
					...
					var ele = "<tr><td>" + data.channelId
					+ "</td><td>" + data.product + "</td><td>"
					+ data.newVister + "</td><td>" + data.registUser
					+ "</td><td>" + data.activeUser + "</td><td>"
					+ data.rechargeUser + "</td><td>" + data.rechargeAmount
					+ "</td><td>" + data.xhAmount + "</td><td>"
					+ data.fcRate + "</td><td>" + data.platformIncome
					+ "</td><td>" + data.fcIncome + "</td><td>" + data.platformTerminal + "</td>";
					$("#data").append(ele);
		}
	}
	...
	var showData = function(pageNumber, pageSize) {
		$("#data").empty();
		$.post("${path}/game/operative/gameChannelOperate/list.do", {
			...
			if (null != json && undefined != json) {
				...
				if (null == infoData || undefined == infoData
						|| 0 >= infoData.length) {
					$("#data").append("<tr><td colspan=\"13\">没有数据</td></tr>");
				} else {
					for (var i = 0; i < infoData.length; i++) {
						
						var ele = {
								channelId : infoData[i].channelId,
								product : infoData[i].product,
								newVister : infoData[i].newVister,
								registUser : infoData[i].registUser,
								activeUser : infoData[i].activeUser,
								rechargeUser : infoData[i].rechargeUser,
								rechargeAmount : infoData[i].rechargeAmount,
								xhAmount : infoData[i].xhAmount,
								fcRate : infoData[i].fcRate,
								platformIncome : infoData[i].platformIncome,
								fcIncome : infoData[i].fcIncome,
	...
	var exportToExcel = function() {
		...
		$.ajax({
			url : "${path}/game/operative/gameChannelOperate/export.do",
			type : "POST",
			data : {
				channelId : $("#channelId").val(),
				platformId:$("#platformId").val() == -999 ? "" : $("#platformId").val(),
			...
			success : function(response, status, request) {
				...
				window.location = "${path}/game/operative/gameChannelOperate/download.do?uuid="
						+ response.uuid;
			}
		});
	}
