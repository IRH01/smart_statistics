								    ...
								    <div class="select-fr" style="float:left;">
								    	<table style="margin-left:5px;">
									    	<tr>
										    	...
										    	<td><input id="developers" class="input" style="width:100px;"></td>
									    	...
									    	<tr>
									    		...
									    		<td>
									    			<select id="rechargeWay"  class="select" style="width:100px;" onchange="$('#rechargeWay').attr('title',$('#rechargeWay').find('option:selected').text());">
		                                                 <option value="-999">请选择</option>
		                                                 <option value="1">微信</option>
		                                                 <option value="2">支付宝</option>
		                                                 <option value="3">乐盈币</option>
		                                                 <option value="4">银联</option>
		                                                 <option value="5">优贝</option>
		                                                 <option value="6">CC电子钱包</option>
		                                                 <option value="7">CC点卡</option>
		                                                 <option value="9">mol电子钱包</option>
		                                                 <option value="10">mol点卡</option>
		                                                 <option value="11" title="越南CardChargePay">越南CardChargePay</option>
											...
											<div class="sortHandle div-sum">
												<span>游戏充值订单金额(VND)：
													<span id="gameRcgSummaryAmount">
														<c:choose>
															<c:when test="${not empty gameRcgSummary}"><fmt:formatNumber type="number" value="${gameRcgSummary}" pattern="0.00"/></c:when>
												...
												<span>&nbsp;&nbsp;&nbsp;&nbsp;乐盈币充值订单金额(VND)：
													<span id="lybRcgSummaryAmount">
														<c:choose>
															<c:when test="${not empty lybRcgSummary}"><fmt:formatNumber type="number" value="${lybRcgSummary}" pattern="0.00"/></c:when>
		       												<c:otherwise>0</c:otherwise>
														</c:choose>
													</span>
												</span>
												<span>&nbsp;&nbsp;&nbsp;&nbsp;剩余乐盈币金额(VND)：
													<span id="remainLybSummaryAmount">
														<c:choose>
															<c:when test="${not empty remainLybSummary}"><fmt:formatNumber type="number" value="${remainLybSummary}" pattern="0.00"/></c:when>
		       												<c:otherwise>0</c:otherwise>
														</c:choose>
													</span>
												</span>
												...
												<table class="tableList1">
													<thead>
														<tr>
															...
															<th style='min-width:75px;'>订单金额（VND）<img class='order' id='amountOrder' src='${path}/img/arrow_disable.png' onclick='orderchange(this);' lang='0'></img></th>
		...
		<script type="text/javascript">
			...
			var addTbRow = function(data,isTotal){
				if(null != data && undefined != data && "" != data){
					var ele="<tr><td >orderNo</td><td>payStatus</td><td>userId</td><td>nickName</td><td>channelId</td><td>orderTypeName</td><td>rechargeWay</td><td>developers</td><td>platformName</td><td>serverName</td><td>amount</td><td style=\"display:none;\">fee</td><td style=\"display:none;\">receipt_amount</td><td>createTime</td><td>payTime</td><td>paymentOrderNo</td><td>tradeNo</td></tr>";
					ele = ele.replace("orderNo",data.orderNo).replace("payStatus",data.payStatus).replace("userId",data.userId).replace("nickName",data.nickName).replace("channelId",data.channelId).replace("orderTypeName",data.orderTypeName).replace("rechargeWay",data.rechargeWay).replace("developers",data.developers).replace("platformName",data.platformName).replace("serverName",data.serverName).replace("amount",data.amount).replace("fee",data.fee).replace("receipt_amount",data.receipt_amount).replace("createTime",data.createTime).replace("payTime",data.payTime).replace("paymentOrderNo",data.paymentOrderNo).replace("tradeNo",data.tradeNo);
					$("#data").append(ele);
				}
			}
