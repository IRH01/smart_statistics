...
;(function ($) {
    //默认参数
    var PARAMS;
    var DEFAULTPARAMS = { Title: "弹出层", Content: "", Width: 400, Height: 300, URL: "", ConfirmFun: new Object, CancelFun: new Object };
    var ContentWidth = 0;
    var ContentHeight = 0;
    $.DialogSdk = {
        //弹出提示框
        ...
        //提示确认框
        Prompt: function (params) { Show(params, "Prompt"); },
        //警告弹出框
        Warning: function (params) { Show(params, "Warning"); },
        //成功弹出框
        Success: function (params) { Show(params, "Success"); },
        //错误弹出框
        Error: function (params) { Show(params, "Error"); },
        //弹出引用其他URL框
        Dialog: function (params) { Show(params, "Dialog") },
        //关闭弹出框
        Close: function () {
            $("#DialogSdkLayer,#DialogSdk").remove();
        }
    ...
    function Show(params, caller) {
        ...
        var Content = [];
        Content.push("<div id=\"DialogSdkLayer\"></div>");
        Content.push("<div id=\"DialogSdk\" style=\"width:" + PARAMS.Width + "px;height:" + PARAMS.Height + "px;left:" + positionLeft + "px;top:" + positionTop + "px;\">");
        Content.push("    <div id=\"Title\" class=\"defalut-title\"><span class=\"titleName\">" + PARAMS.Title + "</span><span id=\"Close\">&#10005;</span></div>");
        ...
        if (caller == "Dialog") {
            ...
            Content.push("                    <input type=\"button\" id=\"btnDialogSdkConfirm\" value=\"确定\" />");
            if (caller == "Confirm") {
                Content.push("                    <input type=\"button\" id=\"btnDialogSdkCancel\" value=\"取消\" />");
            }
            Content.push("                </td>");
            Content.push("            </tr>");
            Content.push("        </table>");
    ...
    function SetDialogEvent(caller) {
        //添加按钮关闭事件
        $("#DialogSdk #Close").click(function () { $.DialogSdk.Close();});
         //添加ESC关闭事件
        ...
        $(window).resize(function(){
            ...
            $("#DialogSdk").css({"top":positionTop+"px","left":positionLeft+"px"});
        });
        $("#DialogSdk #Title").DragSdk($("#DialogSdk"));
        if (caller != "Dialog") {
            $("#DialogSdk #btnDialogSdkConfirm").click(function () {
                $.DialogSdk.Close();
        ...
        if (caller == "Confirm") {
            $('#Title').attr('class','confirm-title')
            $("#DialogSdk #btnDialogSdkCancel").click(function () {
                $.DialogSdk.Close();
        ...
        if(caller == "Prompt"){
            $('#Title').attr('class','Prompt-title')
        }
        if(caller == "Warning"){
            $('#Title').attr('class','Warning-title')
        }
        if(caller == "Success"){
            $('#Title').attr('class','Success-title')
        }
        if(caller == "Error"){
            $('#Title').attr('class','Error-title')
        }
    }
})(jQuery);
//拖动层
